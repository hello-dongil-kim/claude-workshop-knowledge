...
- **STOP PROTOCOL Phase B:** 퀴즈 채점·`progress.md` 갱신 후 **확인 없이 같은 턴에서 다음 블록 자동 진행**. "다음" 수동 타이핑 마찰 제거(선택 B6·B7·심화 B8·트랙 마지막만 1회 확인 — 특히 B8은 비용·설치로 필수 확인, "그만/멈춰"로 언제든 정지).
- **B2 단축키 표기 제거:** grep 설명의 "통합 Ctrl+F"·"AI가 Ctrl+F로" 은유를 키 없는 표현("글자 그대로 찾아주는 도구", "글자 검색")으로 치환. obsidian 편 단축키 제거와 정합.

### Added (추가)

- 전 9블록(B0~B8) EXPLAIN 상단에 **🎙️ 설명 대본** 콜아웃(낭독용 요약, 세부는 물을 때만).
- SKILL "절대 하지 않을 것"에 **reference 밖 단축키·명령 지어내기 금지** 가드레일(확실한 진입점=명령 팔레트).

> 근거: obsidian 편과 공통 구동 피드백(장황·무효 단축키·자동진행) 동반 반영. 두 편 STOP PROTOCOL 동기화.

## [2026-06-30] — council 2차 평가 반영 (정확성·페다고지)

### Changed (변경)

- **정확성 정정 (중요):** "기본 트랙 = 설치·키·비용 0" → **"추가 설치·키·비용 0 (이미 쓰는 Claude Code 외)"**. Claude Code 자체는 계정/사용량 비용이 있음을 명시(README·SKILL·B0·safety-checklist). council 지적 — "무비용" 과장 제거.

### Added (추가)

- **B2:** grep 평이 은유("통합 Ctrl+F") + 에이전트 검색 **실패 사례**(어휘 불일치 "사과↔애플") + **"언제 벡터로 가나" 현실 기준 표**(수백=에이전트 / 수만+=벡터). → B8 진입 동기 선명화.
- **B5 캡스톤:** 질문 6→8 추가 — ⑦지속가능성("6개월 뒤에도 유지될 이유?") ⑧일상 루틴 연결. + 각 결정에 **B0 핵심원리6 재호출**(spaced repetition).
- **B1:** 안티패턴("폴더만 예쁜데 검색 안 되는 Vault") + **보안 카드**("AI에 넘겨도 되는/안 되는 지식" 데이터 분류, 민감 노트 검색 제외).

> 근거: `00_inbox/2026-06-30_워크샵-연작-개선점-평가_council_v0.1.0.md` (3채널 ~9.2/10, "완성형, 갈아엎지 마라" + 가치있는 보강만 반영). 직무사례집·페르소나진단 등은 강의 운영 영역이라 repo 미반영.
