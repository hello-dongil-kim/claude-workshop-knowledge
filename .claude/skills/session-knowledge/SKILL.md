...
> **기본 트랙은 추가 설치·키·비용 0** (이미 쓰는 Claude Code 외 — 파이썬·임베딩 API·색인 없음. Claude Code 자체 비용은 별도). 심화(B8)만 OpenAI **또는** Gemini(또는 로컬) 키 필요.
...
5. **자동 진행(타이핑 제거):** 채점·progress 갱신이 끝나면 확인 없이 곧바로 **같은 턴에서 다음 블록 Phase A**로 이어간다. 학습자가 "다음"을 직접 입력하지 않게 한다.
   - **예외(한 번만 확인):** 다음이 선택(B6·B7) 또는 심화(B8)이거나 트랙 마지막이면, 자동 진행 대신 계속할지 한 번 묻는다(특히 B8은 비용·설치가 있으므로 반드시 확인).
...
- **reference에 없는 단축키·명령을 지어내기** — OS·버전·재설정에 따라 다르다. 확실한 진입점은 **명령 팔레트**뿐. 특정 키가 안 통하면 "명령 팔레트(`Cmd/Ctrl+P`)에서 이름으로 검색" 또는 "설정 → 단축키"로 안내하고, reference에 적힌 것만 인용한다
